...
import { sendExpertWelcomeEmail, sendExpertRemovalEmail } from '../utils/mailer.js';
...
router.patch('/users/:id/suspend', async (req, res) => {
    ...
    try {
        ...
        await db.query(`UPDATE users SET is_suspended=$1 WHERE id=$2`, [!!suspended, id]);
...
router.delete('/providers/:id', async (req, res) => {
    ...
    try {
        ...
        const expertName = [expert.first_name, expert.last_name].filter(Boolean).join(' ');
        ...
        await db.query(`UPDATE chat_messages SET sender_id=NULL WHERE sender_id=$1`, [id]);
        
        // Reviews: Producer name is already in the database as 'removed' expert or nullify
        await db.query(`UPDATE reviews SET producer_id=NULL WHERE producer_id=$1`, [id]);
        
        // Transactions: Salary/Job payout records must be kept but decoupled from the user
        await db.query(`UPDATE transactions SET expert_id=NULL WHERE expert_id=$1`, [id]);
        ...
        await db.query(
            `INSERT INTO admin_activity_log (admin_id, action, target_id, details)
             VALUES ($1, $2, $3, $4)`,
            [req.user.id, 'REMOVE_EXPERT', id, JSON.stringify({ name: expertName, email: expert.email, reason })]
        );

        await db.query('COMMIT');

        // 6. Send email (non-blocking)
        sendExpertRemovalEmail({ name: expertName, email: expert.email, reason }).catch(console.error);

        res.json({ success: true, message: `Expert ${expertName} has been permanently removed.` });
    } catch (err) {
        await db.query('ROLLBACK');
        console.error('Admin remove expert hard delete error:', err);
        res.status(500).json({ error: 'Failed to permanently remove expert account' });
    }
});
...
router.get('/providers/:id/stats', async (req, res) => {
    try {
        ...
        const hasBank = bankRows.length > 0 && bankRows[0].bank_account_number && bankRows[0].ifsc_code;
...
router.post('/providers', async (req, res) => {
    const { fullName, email, password, specialization, machineTypes, serviceCity, qualification, yearsOfExperience, phone } = req.body;
    ...
    try {
        ...
        if (existing.rows.length > 0) {
            return res.status(409).json({ error: 'An account with this email already exists and is currently active.' });
        }
        ...
        const userRes = await db.query(
            ...
            [email.toLowerCase(), passwordHash, firstName, lastName, phone || null, serviceCity.trim()]
        );
        ...
        if (machineTypes) {
            const types = machineTypes.split(',').map(s => s.trim()).filter(Boolean);
            skills.push(...types);
        }
        ...
        const expertName = [user.first_name, user.last_name].filter(Boolean).join(' ');

        // Send welcome email — non-blocking, failure doesn't abort account creation
        let emailSent = true;
        try {
            await sendExpertWelcomeEmail({ name: expertName, email: user.email, password });
        } catch (emailErr) {
...
router.post('/payments/release-salary', async (req, res) => {
    ...
    try {
        ...
        if (expert.status !== 'approved') {
            return res.status(400).json({ error: `Expert status is ${expert.status?.toUpperCase()}. Salary only releasable for APPROVED experts.` });
        }

        if (!expert.bank_account_number || !expert.ifsc_code) {
            return res.status(400).json({ error: 'Expert has not added bank details yet. Salary cannot be released.' });
        }

        if (Number(amount) <= 0) {
            return res.status(400).json({ error: 'Salary amount must be greater than 0 (Bronze level or above).' });
        }
...
router.get('/analytics/overview', async (req, res) => {
    try {
        ...
        const [usersRes, expertsRes, jobsRes, revenueRes] = await Promise.all([
            ...
            db.query(`SELECT COALESCE(SUM(COALESCE(base_amount, provider_price, amount)), 0) AS total FROM transactions WHERE status IN ('completed', 'paid')`)
        ]);
